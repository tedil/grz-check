...
fn check_single_fastq(path: &Path, length_check: ReadLengthCheck, pb: &ProgressBar) -> FileReport {
    ...
    // explicitly drop the reader to be able to finalize the hasher
    drop(reader);
...
fn create_jobs_from_raw_input(
    paired_raw: &[String],
    single_raw: &[String],
) -> anyhow::Result<(Vec<FastqCheckJob>, u64)> {
...
pub fn run_check(
    ...
    if continue_on_error {
        ...
        // fail-fast
        let result: anyhow::Result<Vec<PairReport>, EarlyExitError> = jobs
            .into_par_iter()
...
fn write_report(reports: &[PairReport], output_path: &Path) -> anyhow::Result<()> {
    ...
    for report in reports {
        ...
        writer.serialize(ReportRecord {
            path: report.fq1_report.path.to_string_lossy().to_string(),
            status: fq1_status.to_string(),
            num_reads: fq1_stats.map(|s| s.num_reads),
            read_length: fq1_stats.map(|s| s.read_length),
            checksum: report.fq1_report.checksum.clone(),
            errors: all_errors.join("; "),
        })?;
        ...
        if let Some(fq2_report) = &report.fq2_report {
            ...
            writer.serialize(ReportRecord {
                path: fq2_report.path.to_string_lossy().to_string(),
                status: fq2_status.to_string(),
                num_reads: fq2_stats.map(|s| s.num_reads),
                read_length: fq2_stats.map(|s| s.read_length),
                checksum: report.fq2_report.as_ref().and_then(|r| r.checksum.clone()),
                errors: all_errors.join("; "),
            })?;
        }
    }
...
#[cfg(test)]
mod tests {
    use crate::fastq::{ReportRecord, run_check};
